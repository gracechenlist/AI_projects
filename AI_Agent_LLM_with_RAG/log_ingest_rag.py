...
RAG_VECTOR_FILE = BASE_DIR / "rag_vectors.json"
RAG_TOP_K = 3
...
def build_or_load_vector_store(client: OpenAI) -> list[dict]:
    """
    Embeds knowledge base records and saves them locally.
    Delete rag_vectors.json if you update rag_knowledge_base.json and want to rebuild embeddings.
    """
    kb_items = load_knowledge_base()

    if RAG_VECTOR_FILE.exists():
        with RAG_VECTOR_FILE.open("r", encoding="utf-8") as f:
            return json.load(f)
    ...
    with RAG_VECTOR_FILE.open("w", encoding="utf-8") as f:
...
def retrieve_similar_incidents(client: OpenAI, error_records: list[dict], top_k: int = RAG_TOP_K) -> list[dict]:
    ...
    2. Compare to knowledge base embeddings
    3. Return the most similar past incidents
    """
    if not error_records:
        return []

    vector_store = build_or_load_vector_store(client)
...
def main():
    ...
    print(f"RAG vector store: {RAG_VECTOR_FILE}")
