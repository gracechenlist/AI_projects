...
def search_similar_logs(query_text: str, index=None, metadata=None, k: int = RAG_TOP_K) -> list[dict]:
    """
    FastAPI-compatible helper.

    Takes a single log/error text, embeds it, searches FAISS, and returns
    the top-K similar incidents from metadata.
    """
                    ...
                    <table width="900" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff; border-radius:8px; overflow:hidden;">
                        <tr>
                            <td style="background:#2d6cdf; color:#ffffff; padding:20px; text-align:center; font-size:24px; font-weight:bold;">
                                Log Report with RAG AI Analysis (FAISS version)
                            </td>
                        </tr>
